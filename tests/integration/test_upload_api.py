...
_HAS_LLM = getattr(settings, "ci_mode", False) or (bool(settings.llm_api_key) and settings.llm_api_key != "dummy")
...
class TestUploadZip:
    ...
    def test_upload_zip_skips_pycache(self):
        zip_content = _make_zip({
            "src/main.py": b"print('main')",
...
class TestFilesWorkspaceId:
    def test_files_with_workspace_id(self):
        client = TestClient(app)
        ...
        # 用 workspace_id 列出文件
        files_resp = client.get(f"/api/files?workspace_id={ws_id}")
        assert files_resp.status_code == 200
        files = files_resp.json()["files"]
        assert any(f["name"] == "test.py" for f in files)

    def test_files_content_with_workspace_id(self):
        client = TestClient(app)
        upload_resp = client.post(
            "/api/upload",
            files={"file": ("hello.py", b"print('hello')", "text/plain")},
    ...
    def test_invalid_workspace_id(self):
        client = TestClient(app)
        resp = client.get("/api/files?workspace_id=nonexistent")
...
class TestChatWorkspaceId:
    @pytest.mark.skipif(not _HAS_LLM, reason="external dependency unavailable: LLM API key")
    def test_chat_with_workspace_id(self):
        client = TestClient(app)
        upload_resp = client.post(
            "/api/upload",
